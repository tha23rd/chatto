...
<ReleaseFeatureGrid>
  ...
  <ReleaseFeatureCard title="Interactive integration messages">
    Bots and tools can attach generic buttons to messages and consume durable, idempotent
    invocations. Chatto owns rendering, room authorization, and delivery while each
    integration keeps its application-specific state and behavior.
  </ReleaseFeatureCard>
...
- Upstream 0.5 removes `compatibility.protocol_capabilities`, but this distribution retains
  it for fork-specific protocols that a shared software version cannot identify. Integrations
  targeting this distribution should keep capability checks for those protocols and use an
  explicit supported-release table for upstream features. Regenerate clients from this
  distribution's protobufs: fields added by upstream may use distribution-specific tags to
  preserve wire compatibility with already-published fork clients.
