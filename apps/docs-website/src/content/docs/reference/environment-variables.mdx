...
<EnvVar name="CHATTO_AUTH_PROVIDERS_<index>_<field>" toml="auth.providers">
  Counted environment-variable equivalent of repeated `[[auth.providers]]` tables. Indexes start at `0` and must not have gaps. Supported fields are `ID`, `TYPE`, `LABEL`, `CLIENT_ID`, `CLIENT_SECRET`, `ISSUER_URL`, `SCOPES`, `REQUEST_EMAIL`, `AUTO_PROVISION`, `ROLE_CLAIM`, `ROLE_CLAIM_ALLOWED_ROLES`, `ROLE_CLAIM_MODE`, and `PROVIDER_OPTIONS_<KEY>`.
</EnvVar>
...
# Optional direct OIDC role synchronization.
scopes = ["openid", "profile", "roles"]
role_claim = "roles"
role_claim_allowed_roles = ["moderator"]
role_claim_mode = "additive"
```
...
OIDC providers can synchronize direct role claim values with `role_claim`. Set `ROLE_CLAIM_ALLOWED_ROLES` as a comma-separated list and `ROLE_CLAIM_MODE` to `additive` or `reconcile`. See [External Login Providers](/guides/integrations/external-login-providers/#oidc-role-claims) for the security and wildcard behavior.
