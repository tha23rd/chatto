...
<Aside type="caution">
  Chatto's public API is experimental while Chatto is pre-1.0. We prefer additive changes, but breaking changes can happen in the `0.x` line. Pin server versions for production integrations, test the releases you support, and read release notes before upgrading. See [API Compatibility](/guides/integrations/api-compatibility/).
</Aside>
...
| Add buttons to messages and consume invocations | `MessageService` and `MessageActionService` |
...
<CardGrid>
  ...
  <LinkCard title="Message Actions" href="/guides/integrations/message-actions/" description="Add reusable buttons to bot and integration messages." />
