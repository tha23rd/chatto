...
When a Chatto frontend connects to another Chatto server, it opens that server's `/oauth/authorize` page in a separate browser window and uses PKCE to mint a bearer token. Passwords, passkeys, and external-provider sign-in stay on the remote server's own pages. After sign-in, the remote server redirects a short-lived authorization code to the trusted frontend origin; the main client exchanges the code and stores the resulting token.
...
Chatto 0.5 and newer also trust the exact official Chatto Desktop callback,
`chatto://desktop/servers/callback`, without an OAuth redirect-origin setting.
Restrictive CORS configurations must still include `chatto://desktop` so the
desktop app can call the API and open its realtime connection. Desktop access
uses the same PKCE flow and per-origin consent as hosted frontends.
...
`core.secret_key` is not a message-encryption KEK. Rotating it invalidates bearer tokens, cookie-session handles, OAuth authorization codes, pending account-flow credentials, and existing invite links, but it does not rotate or invalidate the KEK records in `ENCRYPTION_KEYS`. Invite-link records remain in `EVT`, so authorised administrators can copy newly derived links after rotation.
