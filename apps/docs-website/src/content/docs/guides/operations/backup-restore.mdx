...
| [Search index](/guides/operations/search/) | Privileged plaintext-derived local cache. Protect its volume separately or rebuild it from retained `EVT` history. |
| S3-backed assets and projection snapshot generations | Stored outside NATS; protect them through the deployment's S3 backup policy. The encrypted snapshot pointer remains in `RUNTIME_STATE`, but is disposable without its generation object |
...
<Aside type="note">
  Projection snapshots are never required to restore server state. NATS-backed snapshots and their revisioned pointers remain reusable after restore when the same `core.secret_key` is retained and the snapshot has not expired. Missing or expired generations and key changes only cause the affected projection cohort to replay `EVT`. Snapshot retention defaults to seven days. NATS enforces it through Object Store TTL; S3 deployments use Chatto's marked-object expiry by default or an operator-managed lifecycle policy.
</Aside>
