...
4. Other non-muted room members receive a call-start notification and see active call indicators update in real time.

The first person to select **Start call** and join creates the call and a fresh E2EE key. Chatto sends one notification to every other current room member unless they have muted the room; later joins do not send more call-start notifications. Do Not Disturb keeps the notification in the notification centre but suppresses its sound and Web Push. When the last person leaves, the call ends and Chatto shreds that key.

Screen sharing can include audio when the capture engine supports the selected surface. In Chrome, select a browser tab and enable **Share tab audio** in the browser picker. In the Windows desktop client, select an entire display and enable the picker's system-audio control to include Windows system audio. To share a window, select it and enable the picker's application-audio control; Chatto requests audio from the application that owns the window and its process tree. Picker wording varies with the WebView2 runtime.

WebView2 may offer all-system audio when application capture is unavailable. Chatto does not silently broaden a selected-window share: it removes that fallback audio before publishing, keeps the window video live, and warns the presenter that the window is being shared without sound. Browser-tab scoped audio and entire-display system audio remain available.
...
Reloading the page while in a call reconnects you automatically: the client remembers which room call you were in and rejoins as soon as it confirms the call is still active, so a refresh no longer drops you out of the conversation. If the call ended while the page was closed, no new call is started on your behalf.
...
- **Call keybinds** — Assign per-device push-to-talk, push-to-mute, mute/deafen, camera, screen-share, and leave-call controls in **Settings → Keybinds**. Browser bindings work while the Chatto tab is active; Windows desktop bindings work system-wide while connected to a call.
...
- **Screen sharing** — Share a screen, window, or browser tab, with capture audio where the platform supports it
- **Video pop-out** — Float a camera or screen-share feed while using another window; the Windows desktop client provides a movable, resizable window with minimise, maximise, and close controls
- **Active call indicators** — Rooms with active calls show a call indicator in the room list
- **Call-start notifications** — Other non-muted room members receive one notification when a new call session starts
- **Speaking indicators** — Active speakers show a neutral inline volume indicator
