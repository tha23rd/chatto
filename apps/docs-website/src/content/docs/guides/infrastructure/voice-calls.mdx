...
Screen sharing can include audio when the capture engine supports the selected surface. In Chrome, select a browser tab and enable **Share tab audio** in the browser picker. In the Windows desktop client, enable **Share audio** to pair an entire display or selected window with Windows system audio. Selecting a window limits the picture, not the sound: other applications may be audible too. Chatto asks WebView2 to remove its own call playback from that capture to prevent feedback, but older WebView2 runtimes may not support the window-audio or filtering controls.
