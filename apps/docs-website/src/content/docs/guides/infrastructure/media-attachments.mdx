...
| Video playback with seeking | Enable video processing; HLS works with either storage backend. |
...
Video processing is optional and disabled by default. When enabled, Chatto requires `ffmpeg` and `ffprobe`, stores the original upload, generates a thumbnail, and packages temporary renditions into durable six-second HLS segments. Chatto generates the small HLS playlists on demand rather than storing them.
