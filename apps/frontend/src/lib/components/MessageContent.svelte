...
<script lang="ts">
  ...
  // Render markdown, wrap valid mentions, swap known custom-emoji shortcodes
  // for their images, and finally turn timestamp tokens into interactive
  // controls. Reactive inputs are passed explicitly so the await block reruns
  // when the emoji catalog, locale, or time settings change.
...
<div class="prose max-w-none min-w-0" dir="auto" role="presentation" onclick={handleContentClick}>
  {#await render(body, members, roleHandles, edited, viewerLogin, customEmojis, timestampSettings, timestampLocale)}
    {body}
  {:then html}
    <MarkdownHtml {html} />
  {:catch error}
