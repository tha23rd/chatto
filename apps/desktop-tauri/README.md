...
`apps/desktop-tauri/updater-public-key.txt`. GitHub's built-in workflow token creates
...
`apps/desktop-tauri/src-tauri/target/release/bundle/nsis`. If `CARGO_TARGET_DIR` is
...
./apps/desktop-tauri/scripts/verify-package.ps1 -OutputDirectory $evidence
```
...
```powershell
./apps/desktop-tauri/scripts/measure-resources.ps1 `
  -ProcessId 1234 `
