...
| `notification.{userId}.{notificationId}` | Pending notification record (protobuf `Notification`) for DM messages, @mentions, replies, all-message subscriptions, and voice calls started by another room member. A call-start row carries `voice_call_started_details` beside a legacy-compatible `room_message` payload so older replicas can process it during rolling upgrades. Uses per-key 90-day TTL. Internal `NotificationCreatedEvent` / `NotificationDismissedEvent` signals on `live.sync.user.{userId}.*` trigger authoritative notification projection replacements; DND keeps the record but marks the live creation transition silent and skips push delivery. |
