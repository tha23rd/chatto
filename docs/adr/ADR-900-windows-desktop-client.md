...
AudioWorklet source generated by its dependency. Direct remote and inline
scripts remain blocked, and the normal web CSP does not inherit this exception.
Re-evaluate the exception if the dependency exposes a same-origin worklet asset
before enhanced suppression leaves experimental status.
