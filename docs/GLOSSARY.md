...
**Webhook user** — Synthetic, non-human user of kind `USER_KIND_WEBHOOK` that backs a *Channel Webhook* and authors its messages. Passwordless and excluded from the member directory, login resolution, and mention autocomplete. See [FDR-031](fdr/FDR-031-channel-webhooks.md).
